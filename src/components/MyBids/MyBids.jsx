  ...
  useEffect(() => {
    if (bids.length > 0) {
      ...
      productIds.forEach((productId) => {
        fetch(
          `https://smart-deals-server-seven-gamma.vercel.app/products/${productId}`
        )
          .then((res) => res.json())
          .then((data) => {
            setProduct((prev) => ({
    ...
    <div className="w-full bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mt-11 max-w-11/12 mx-auto">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="btn px-4 py-1.5 text-xs font-medium text-red-700 bg-red-50 border border-red-600 rounded hover:bg-red-100 transition-colors"
                      >
                        Remove Bid
                      </button>
                    </div>
      ...
      {bids.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500 text-sm">No bids yet for this product.</p>
        </div>
      )}
    </div>
  );
